...
<script>
if(1) {
    let url = 'https://pjshuoshuo2.zeabur.app'
    fetch(url + '/api/memo?creatorId=1&tag=说说&limit=30').then(res => res.json()).then(data => { // 注意修改域名和用户id
        ...
        items.forEach(item => {
            html += `<div class="talk_item"><div class="talk_meta"><div class="talk_meta2"><img class="no-lightbox no-lazyload avatar" src="https://pic-alex.pages.dev/cover/fm1.jpeg"><div class="info"><span class="talk_nick">Alex${icon}</span><span class="talk_date">${item.date}</span></div></div><div class="talk_tag"><span class="talk_tag">${item.tag}</span></div></div><div class="talk_content">${item.content}</div><div class="talk_bottom"><div></div><a href="javascript:;"onclick="goComment('${item.text}')"><span class="icon"><i class="fa-solid fa-message fa-fw"></i></span></a></div></div>` // 注意修改头像链接和名称
        })
